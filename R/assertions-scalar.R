...
#' @param nona if \code{TRUE}, \code{NA} values will not be considered flags
#' @name scalar
#' @aliases NULL
NULL
...
#' see_if(is.flag(NA, nona = TRUE))
#' see_if(is.flag(NA, nona = FALSE))
#'
is.flag <- function(x, nona = FALSE)
  if(nona) {
    !is.na(x) && is.logical(x) && length(x) == 1
  } else {
